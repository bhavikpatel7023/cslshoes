...
class AccountsReceivableSummaryII(ReceivablePayableReport):
	def run(self, args):
	...
	def get_data(self, party_naming_by, args):
		...
		for party, party_dict in iteritems(partywise_total):
			...
			party_dict.total =  party_dict.range1 + party_dict.range2 + party_dict.range3 + party_dict.range4 + party_dict.range5
			
			row += [
				party_dict.invoiced_amt, paid_amt, party_dict.credit_amt, party_dict.outstanding_amt,
				party_dict.range1, party_dict.range2, party_dict.range3, party_dict.range4, party_dict.range5, party_dict.total
			]
...
def execute(filters=None):
	...
	return AccountsReceivableSummaryII(filters).run(args)
